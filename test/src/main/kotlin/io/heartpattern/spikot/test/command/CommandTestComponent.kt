...
import io.heartpattern.spikot.command.arguments.remain
import io.heartpattern.spikot.command.arguments.single
...
class CommandTestComponent : Component() {
    override fun onEnable() {
        plugin.registerCommand("test") {
            ...
            failureHandler<MissingCommandFailure> { node, _ ->
                node.usage.forEach {
                    (sender as Player).sendMessage(it)
                }
            }
        }
    }
