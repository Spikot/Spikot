...
class SpikotPluginMap<V>(
    private val default: (SpikotPlugin) -> V
) : RestrictedKeyMap<SpikotPlugin, V>() {
    override val keys: Set<SpikotPlugin>
        get() = Bukkit.getPluginManager().plugins.asSequence().filterIsInstance<SpikotPlugin>().toSet()

    override fun default(key: SpikotPlugin): V {
        return default.invoke(key)
    }
}