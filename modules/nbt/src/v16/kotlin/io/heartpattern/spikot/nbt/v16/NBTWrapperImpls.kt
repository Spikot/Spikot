...
data class ListTagImpl(override val rawTag: NBTTagList) : ListTag {
    ...
    override val elementType: TagType<*>
        get() = TagType.ofId(rawTag.typeId.toInt()) ?: throw IllegalStateException("Unknown tag id: ${rawTag.typeId}")
}
...
data class CompoundTagImpl(override val rawTag: NBTTagCompound) : CompoundTag {
    ...
    override fun getTagType(key: String): TagType<*> = TagType.ofId(rawTag[key]?.typeId?.toInt() ?:0)
        ?: throw IllegalStateException("Unknown tag id: ${rawTag[key]?.typeId?.toInt() ?: 0}")
